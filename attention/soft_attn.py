...
def attn_window(scope, h_dec, N):
    ...
    with tf.variable_scope(scope):
        ...
        gx = (self._W + 1) / 2 * (gx_ + 1)
        gy = (self._H + 1) / 2 * (gy_ + 1)
        sigma2 = tf.exp(log_sigma2)
        gamma = tf.exp(log_gamma)
        delta = (max(self._W, self._H) - 1) / (N - 1) * tf.exp(log_delta) # batch x N
...
class ReadSoftAttn(ReadInterface):
    ...
    def read(self, x, x_hat, h_dec_prev):
        ...
        with tf.variable_scope('read'):
            ...
            Fx, Fy, gamma, visualization_params = attn_window("soft_attn", h_dec_prev, self._N)
    ...
    def write(self, h_dec):
        ...
        with tf.variable_scope('write'):
        	...
        	for w_channel in w_channels:
        		...
        		wr = tf.reshape(wr, [batch_size, self._H * self._W])
